using DiffEqFlux: HamiltonianNN
using OrdinaryDiffEq, Test, Zygote, Lux, Random, NNlib

# Checks for Shapes and Non-Zero Gradients
u0 = rand(Float32, 6, 1)

hnn = HamiltonianNN(Chain(Dense(6, 12, relu), Dense(12, 1)))
p, st = Lux.setup(Random.default_rng(), hnn)

@test size(hnn(u0,p,st)[1]) == (6, 1)
Zygote.gradient(p -> sum(first(hnn(u0, p, st))), p)
@test ! iszero(ReverseDiff.gradient(p -> sum(hnn(u0, p, st)), p))
